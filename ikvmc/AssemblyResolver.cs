...
namespace IKVM.Internal
{
    sealed class AssemblyResolver
    {
        ...
        {
            //Get the directory whete the primary runtime assembly is located
            var location = new DirectoryInfo(typeof(object).Assembly.Location);
            Console.Error.WriteLine($"System runtime is located at {location.Parent.FullName}.");
            ...
            if (references != null)
            {
                foreach (string r in references)
                {
                    try
                    {
                        Console.Error.WriteLine($"Testing whether {r} is the core library");
                        if (AssemblyName.GetAssemblyName(r).Name.IsPartOfCore())
                        {
                            Console.Error.WriteLine("Identified as corelib " + r);
                            return LoadFile(r);
                        }
                    }
                    catch
                    {
                        //Retry by appending the directory where the core assemblies are located
                        if (!r.Contains(Path.DirectorySeparatorChar + ""))
                        {
                            try
                            {
                                Console.Error.WriteLine($"Retrying with {location.Parent.FullName + Path.DirectorySeparatorChar + r} and testing if it is the core library");
                                if (AssemblyName.GetAssemblyName(location.Parent.FullName + Path.DirectorySeparatorChar + r).Name.IsPartOfCore())
                                {
                                    Console.Error.WriteLine("Identified as corelib " + r);
                                    return LoadFile(location.Parent.FullName + Path.DirectorySeparatorChar + r);
                                };
                            }
                            catch (Exception ex)
                            {
                                Console.Error.WriteLine($"Could not load the assembly: r due to {ex.Message}");
                            }
                        }
                    }
                }
            }
            foreach (string mscorlib in FindAssemblyPath(RuntimeInfo.ReferenceCoreLibLocation))
            {
                Console.Error.WriteLine("Secondary load: " + mscorlib);
