...
sealed class IkvmcCompiler
{
    ...
    static int Compile(string[] args)
    {
        ...
        /*//Auto-add DLL refernces from the reference folder
        if (!nostdlib)
        {
            foreach (var file in Directory.EnumerateFiles(Path.GetDirectoryName(RuntimeInfo.ReferenceCoreLibLocation)))
            {
                Console.WriteLine($"Adding reference: {file}");
                ArrayAppend(ref toplevel.unresolvedReferences, file);
            }
        }
*/
