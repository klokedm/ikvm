...
sealed class MethodAnalyzer
{
    ...
    private void PatchFieldAccess(TypeWrapper wrapper, MethodWrapper mw, ref ClassFile.Method.Instruction instr, StackState stack)
    {
        ...
        else
        {
            ...
            //XXX: HACK - compare by name instead of reference
            if (cpi.GetFieldType().Name != field.FieldTypeWrapper.Name && !cpi.GetFieldType().IsUnloadable & !field.FieldTypeWrapper.IsUnloadable)
            {
#if STATIC_COMPILER
    ...
    {
        //TODO: This is a quick hack to compare by name instead of by reference; this needs to be further reviewed.
        if (cpi.GetRetType().Name != mw.ReturnType.Name && !cpi.GetRetType().IsUnloadable && !mw.ReturnType.IsUnloadable)
        {
#if STATIC_COMPILER
        ...
        {
            //XXX: HACK AHEAD; it seems type forwarding mangles the assumptions about the types for verificaton. This is a temporary fix to get the OpenJDK to build
            //if(here[i] != there[i] && !here[i].IsUnloadable && !there[i].IsUnloadable)
            if (here[i].Name != there[i].Name && !here[i].IsUnloadable && !there[i].IsUnloadable)
            {
#if STATIC_COMPILER
